...
from research_agent import DEPTH_SETTINGS, MODEL_NAME, PATCH_STATUS, run_research  # noqa: E402
...
with st.sidebar:
    ...
    st.caption(f"Model: `{MODEL_NAME.removeprefix('groq/')}`")
    ...
    if not topic.strip():
        ...
        try:
            with st.spinner("Researching and writing... this can take 30-90 seconds."):
                report = run_research(topic.strip(), depth, api_key)
