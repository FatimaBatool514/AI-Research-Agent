...
# --- Automatic retry when Groq's free-tier limit (tokens per minute) is hit ---
MAX_RETRIES = 8          # how many times to wait-and-retry one request
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            while True:
                try:
                    ...
                    delay = _retry_delay(exc, attempt) if is_groq else None
                    if delay is None:
                        raise
                    attempt += 1
                    print(f"[groq patch] rate limit hit, waiting {delay:.1f}s (retry {attempt})")
                    time.sleep(delay)
