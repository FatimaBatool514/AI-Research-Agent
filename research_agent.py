...
from tools import duckduckgo_search, read_webpage
...
def _prepare_retry(exc, kwargs):
    """After a wrong-tool failure, add a short reminder message to the retry."""
    if "tool_use_failed" in str(exc) and isinstance(kwargs.get("messages"), list):
        if not any(m.get("content") == _TOOL_NUDGE for m in kwargs["messages"] if isinstance(m, dict)):
            kwargs = {**kwargs, "messages": kwargs["messages"] + [{"role": "user", "content": _TOOL_NUDGE}]}
    return kwargs
