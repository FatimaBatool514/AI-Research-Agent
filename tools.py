"""Tools for the agent: web search and reading a web page (both free, no API key).

`build_tools()` creates fresh tools for ONE research run. Each tool has a budget
(for example "max 3 searches"). When the budget is used up, the tool tells the
agent to stop researching and write the report - this keeps the agent from
looping until CrewAI's iteration limit forces an answer.
"""
...
_STOP = "Do NOT use this tool again. Write the final report now using the information you already have."
...
def build_tools(max_searches: int, max_pages: int):
    """Create the agent's two tools for one run, each with its own budget."""
    ...
    def duckduckgo_search(query: str) -> str:
        ...
        if used["searches"] >= max_searches:
            return f"Search limit reached ({max_searches} searches). {_STOP}"
        used["searches"] += 1
        return _search(query)

    @tool("Read Webpage")
    def read_webpage(url: str) -> str:
        """Open a web page and return its text (shortened). Input: one full URL that
        starts with http:// or https:// and came from a search result."""
        if used["pages"] >= max_pages:
            return f"Page limit reached ({max_pages} pages). {_STOP}"
        used["pages"] += 1
        return _read_page(url)

    return [duckduckgo_search, read_webpage]
